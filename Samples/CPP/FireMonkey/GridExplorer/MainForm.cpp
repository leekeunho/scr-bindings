...
void __fastcall TFrmMain::edPathKeyUp(TObject *Sender, WORD &Key,
	...
	if (Key == vkReturn) {
		Fmx::Platform::Platform->SetCursor(this, System::Uitypes::crHourGlass);
		UpdateFolderList();
		Fmx::Platform::Platform->SetCursor(this, System::Uitypes::crDefault);

//		_di_TThreadProcedure methProc
//			(static_cast<TThreadProcedure*>(new TExplorerQueueProc(UIHook)));
...
// ---------------------------------------------------------------------------
void __fastcall TFrmMain::Grid1SetValue(TObject *Sender, const int Col,
	const int Row, const Variant &Value) {
	TFileInfo *f;
	f = dynamic_cast<TFileInfo*>(VariantToObject(Value));
	Data[Row] = f;

}

// ---------------------------------------------------------------------------
void __fastcall TFrmMain::Grid1GetValue(TObject *Sender, const int Col,
	const int Row, Variant &Value) {
	if (UpdateGrid) {
