...
uses
  ...
  DBXMetaDataProvider, DBClient, Generics.Collections, Datasnap.DSClientRest,
  IndyPeerImpl;
...
uses
  DbxInterbase, System.IniFiles;

const
  cnTestConnection = 'DBUtilsTest';
  cnTestRestAllConnection = 'DBUtilsTestRestAll';
  cnTestRestWorkingConnection = 'DBUtilsTestRestWorking';
...
procedure TDBXUtilsTests.VerifyRestConnection(AConnection: TDSRestConnection;
  AConnectionName: string; AFree: Boolean = True; AExtraProc: TProc = nil);
var
  lIni: TIniFile;
begin
  try
    CheckNotNull(AConnection, 'Connection not assigned');
    lIni := TIniFile.Create(ModuleIniFile);
    try
...
procedure TDBXUtilsTests.ClearRestProps(AConnection: TDSRestConnection);
begin
...
procedure TDBXUtilsTests.InitRestConnectionPropsTest;
var
  lIni: TIniFile;
  lHost: string;
  lUrlPath: string;
...
begin
  ...
  try
    ...
    try
      InitRestConnectionProps(lConnection, lHost, lUrlPath, lPort, lProtocol, lContext,
      ...
      InitRestConnectionProps(lConnection, lHost, lUrlPath, lPort);
...
procedure TDBXUtilsTests.InitRestConnectionTest;
var
  lConnection: TDSRestConnection;
begin
  lConnection := TDSRestConnection.Create(nil);
  try
    InitRestConnection(lConnection, cnTestRestAllConnection);
    VerifyRestConnection(lConnection, cnTestRestAllConnection, False);

    ClearRestProps(lConnection);
    InitRestConnection(lConnection, cnTestRestAllConnection, ModuleIniFile);
    VerifyRestConnection(lConnection, cnTestRestAllConnection, False);

    ClearRestProps(lConnection);
    InitRestConnection(lConnection, cnTestRestWorkingConnection);
    ...
    CheckExceptionEx(procedure
      begin
        InitRestConnection(lConnection, cnTestConnection, cnInvalidIni);
      end, EDBXUtils, Format(StrCouldNotInitConnection, [cnTestConnection,
...
procedure TDBXUtilsTests.CreateRestConnectionPropsTest;
var
  lIni: TIniFile;
  lHost: string;
  lUrlPath: string;
...
begin
  ...
  try
    ...
    lConnection := CreateRestConnectionProps(lHost, lUrlPath, lPort, lProtocol, lContext,
      lUserName, lPassword, lProxyHost, lProxyPort, lProxyUserName, lProxyPassword);
    ...
    lUrlPath := lIni.ReadString(cnTestRestWorkingConnection, SUrlPath, SDefaultUrlPath);
    lPort := lIni.ReadInteger(cnTestRestWorkingConnection, SPort, SDefaultPort);
    lConnection := CreateRestConnectionProps(lHost, lUrlPath, lPort);
...
function TDBXUtilsTests.CreateTestCommand(AConnection: TDSRestConnection): TDSRestCommand;
const
...
procedure TDBXUtilsTests.CreateRestConnectionTest;
var
  lConnection: TDSRestConnection;
begin
  lConnection := CreateRestConnection(cnTestRestAllConnection);
  VerifyRestConnection(lConnection, cnTestRestAllConnection);

  lConnection := CreateRestConnection(cnTestRestAllConnection, ModuleIniFile);
  VerifyRestConnection(lConnection, cnTestRestAllConnection);

  lConnection := CreateRestConnection(cnTestRestWorkingConnection);
  ...
  CheckExceptionEx(procedure
    begin
      CreateRestConnection(cnTestConnection, cnInvalidIni);
    end, EDBXUtils, Format(StrCouldNotInitConnection, [cnTestConnection,
      Format(StrAppIniMissing, [cnInvalidIni])]));
end;
