...
procedure TfMain.Form3DCreate(Sender: TObject);
begin
...
procedure TfMain.Form3DDestroy(Sender: TObject);
begin
...
procedure TfMain.FormIdle(Sender: TObject; var Done: boolean);
begin
...
procedure TfMain.lbMeshesChange(Sender: TObject);
var
 LMesh: TMesh;
begin
...
procedure TfMain.rbMeshModeSolidChange(Sender: TObject);
begin
  if rbMeshModeSolid.IsChecked then
    FContextState := TContextState.csSolid;
end;

procedure TfMain.rbNormalMapChange(Sender: TObject);
begin
  FPixelShader := FShaders.PixelMode[pmNormal];
end;

procedure TfMain.rbSpecularMapChange(Sender: TObject);
begin
  FPixelShader := FShaders.PixelMode[pmSpecular];
end;

procedure TfMain.rbAmbientMapChange(Sender: TObject);
begin
  FPixelShader := FShaders.PixelMode[pmAmbient];
end;

procedure TfMain.rbCompositionChange(Sender: TObject);
begin
  FPixelShader := FShaders.PixelMode[pmComposition];
end;

procedure TfMain.rbDiffuseMapChange(Sender: TObject);
begin
  FPixelShader := FShaders.PixelMode[pmDiffuse];
end;

procedure TfMain.rbEffectsChange(Sender: TObject);
begin
  FPixelShader := FShaders.PixelMode[pmEffects];
end;

procedure TfMain.rbMeshModeFrameChange(Sender: TObject);
begin
...
procedure TfMain.cbLightPosAsCameraPosChange(Sender: TObject);
begin
  cbAnimatedLight.Enabled := not cbLightPosAsCameraPos.IsChecked;
end;

procedure TfMain.cbModelChange(Sender: TObject);
var
  i:Integer;
  LItem: TListBoxItem;
begin
  ...
  case cbModel.ItemIndex of
    0: FModel := TModel.CreateFromFile(MODELS_PATH + 'FireMonkey\FireMonkey.DAE');
    1: FModel := TModel.CreateFromFile(MODELS_PATH + 'Station\Station.DAE');
  else
    Assert(False);
  end;
...
procedure TfMain.cpAmbientColorChange(Sender: TObject);
begin
  Viewport.Color := cpAmbientColor.Color;
end;

procedure TfMain.Dummy1Render(Sender: TObject; Context: TContext3D);
begin
  ...
  Context.SetVertexShaderMatrix(-1, FCamera.GetCameraMatrix(Context));
