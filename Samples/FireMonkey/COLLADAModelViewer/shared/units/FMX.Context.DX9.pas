...
begin
  if Index = -1 then
    ...
    begin
      ...
      FAILED(FDevice.SetVertexShaderConstantF(Index + 0, @V, 1));
      V := Vector3D(M.m12, M.m22, M.m32, M.m42);
      FAILED(FDevice.SetVertexShaderConstantF(Index + 1, @V, 1));
      V := Vector3D(M.m13, M.m23, M.m33, M.m43);
      FAILED(FDevice.SetVertexShaderConstantF(Index + 2, @V, 1));
      V := Vector3D(M.m14, M.m24, M.m33, M.m44);
      FAILED(FDevice.SetVertexShaderConstantF(Index + 3, @V, 1));
      Exit;
    end;
